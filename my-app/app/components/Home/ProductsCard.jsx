    ...
    <div className="w-75 group flex flex-col gap-3 p-4 rounded-2xl hover:bg-white/5 transition-colors border border-transparent hover:border-white/5 relative">
      <div className="relative aspect-square w-full overflow-hidden rounded-xl bg-gray-800 shadow-md">
        ...
        <button
          ...
          className={`absolute cursor-pointer top-3 right-3 z-30 flex items-center justify-center rounded-full p-1.5 backdrop-blur-sm transition-colors duration-300
            ${inWishlist ? 'bg-black/40' : 'bg-black/40 '} `}
        >
          <FiHeart
            className={`w-5 h-5 transition-colors duration-300 ${
              inWishlist ? 'text-red-600 ' : 'text-white group-hover:text-green-400'
            }`}
          />
        </button>
