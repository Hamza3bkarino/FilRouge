...
export default function EditProgram() {
  ...
  const handleAIDescription = async () => {
    if (!form.name || !form.level || !form.goal) return alert("Fill all fields!");
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.level.trim()) { newError.level = "Level is required"; valid = false; }
    if (!form.goal.trim()) { newError.goal = "Goal is required"; valid = false; }
    if (!form.duration.trim()) { newError.duration = "Duration is required"; valid = false; }
    if (!form.price || Number(form.price) <= 0) { newError.price = "Valid price is required"; valid = false; }
    ...
    <div className="max-w-3xl mx-auto px-4 py-6">
      ...
      <form onSubmit={handleSubmit} className="bg-gray-900/50 p-6 rounded-xl space-y-4 shadow-md">
        ...
        <div className="flex flex-col">
          <label className="text-xs font-bold uppercase text-gray-400 mb-1">Program Name</label>
          <input
            type="text"
            name="name"
            value={form.name}
        ...
        {/* Image Upload */}
        <div className="flex flex-col">
          <label className="text-xs font-bold uppercase text-gray-400 mb-1">Program Image</label>
          <input
            type="file"
            accept="image/*"
          ...
          {form.image && typeof form.image === 'string' && (
            <img src={form.image} alt="Program" className="w-32 mt-2 rounded" />
          )}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="flex flex-col">
            <label className="text-xs font-bold uppercase text-gray-400 mb-1">Level</label>
            <select
              name="level"
              value={form.level}
              onChange={handleInputChange}
          ...
          <div className="flex flex-col">
            <label className="text-xs font-bold uppercase text-gray-400 mb-1">Goal</label>
            <select
              name="goal"
              value={form.goal}
              onChange={handleInputChange}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="flex flex-col">
            <label className="text-xs font-bold uppercase text-gray-400 mb-1">Duration</label>
            <input
              type="text"
              name="duration"
              value={form.duration}
              ...
              className="bg-gray-800 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500"
            />
            {errors.duration && (<p className="text-red-600 text-[13px]">{errors.duration}</p>)}
          </div>
          <div className="flex flex-col">
